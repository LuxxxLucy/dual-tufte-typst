import { renderBody, formatOptions, makeSelect, makePane } from "./_panes.js";
...
function makeSidePane(side, initialStyle, initialFormat, manifest) {
    ...
    const sideLabel = Object.assign(document.createElement("span"), { textContent: side + ":" });
    const styleSel  = makeSelect(styleOpts,    state.style,  v => { state.style  = v; redraw(); });
    const formatSel = makeSelect(formatOptions, state.format, v => { state.format = v; redraw(); });

    const { pane, body } = makePane([sideLabel, styleSel, formatSel]);
