// Faithful port of the canonical tufte-css homepage.
// Source: https://edwardtufte.github.io/tufte-css/  (Dave Liepmann).
// Iframe-wrapper section is intentionally omitted — it is an HTML-only
// embedding helper with no typesetting analogue.
...
#show: tufte.with(
    ...
    // tufte-original mirrors tufte-css 1.8.0 — the right default for the
    // homepage reproduction. Gallery passes `--input style=<name>` to
    // re-render this same source under every registered style.
...
Tufte CSS was created by #link("http://www.daveliepmann.com")[Dave Liepmann]
and is now an Edward Tufte project. The original idea was cribbed from
#link("https://tufte-latex.github.io/tufte-latex/")[Tufte-LaTeX] and
#link("http://rmarkdown.rstudio.com/tufte_handout_format.html")[R Markdown's Tufte Handout format].
We give hearty thanks to all the people who have contributed to those
projects.

If you see anything that Tufte CSS could improve, we welcome your
contribution in the form of an issue or pull request on the GitHub project:
#link("https://github.com/edwardtufte/tufte-css")[tufte-css]. Please note
the #link("https://github.com/edwardtufte/tufte-css#contributing")[contribution guidelines].
...
Tufte CSS uses `h1` for the document title, `p` with class `subtitle` for
the document subtitle, `h2` for section headings, and `h3` for low-level
headings. More specific headings are not supported. If you feel the urge to
reach for a heading of level 4 or greater, consider redesigning your
document:
...
#quote(attribution: link(
    ...
    _sentences_ which then cumulate sequentially into _paragraphs_, rather
    than the grunts of bullet points. Undergraduate Caltech physics is very
...
of block quotes. In Tufte CSS they are just lightly styled, semantically
correct HTML using `blockquote` and `footer` elements. See page 20 of
#link("https://www.edwardtufte.com/tufte/books_vdqi")[The Visual Display of Quantitative Information]
for an example in print.

#new-thought[In his later books]#sidenote[#link("http://www.edwardtufte.com/tufte/books_be")[#emph[Beautiful Evidence]]],
Tufte starts each section with a bit of vertical space, a non-indented
paragraph, and the first few words of the sentence set in small caps. For
this we use a span with the class `newthought`, as demonstrated at the
beginning of this paragraph. Vertical spacing is accomplished separately
through `<section>` tags. Be consistent: though we do so in this paragraph
for the purpose of demonstration, do not alternate use of header elements
and the `newthought` technique. Pick one approach and stick to it.

== Text

Although paper handouts obviously have a pure white background, the web is
better served by the use of slightly off-white and off-black colors. Tufte
CSS uses `#fffff8` and `#111111` because they are nearly indistinguishable
from their 'pure' cousins, but dial down the harsh contrast. We stick to
the greyscale for text, reserving color for specific, careful use in figures
and images.

In print, Tufte has used the proprietary Monotype Bembo#sidenote[See Tufte's
comment in the #link("http://www.edwardtufte.com/bboard/q-and-a-fetch-msg?msg_id=0000Vt")[Tufte book fonts]
thread.] font. A similar effect is achieved in digital formats with the now
open-source #link("https://github.com/edwardtufte/et-book")[ETBook], which
Tufte CSS supplies with a `@font-face` reference to a `.ttf` file. In case
ETBook somehow doesn't work, Tufte CSS shifts gracefully to other serif
fonts like Palatino and Georgia.
...
#sans[If you prefer sans-serifs, use the `sans` class. It relies on Gill
Sans, Tufte's sans-serif font of choice.]

Links in Tufte CSS match the body text in color and do not change on
mouseover or when clicked. Here is a #link("#")[dummy example] that goes
nowhere. These links are underlined, since this is the most widely
recognized indicator of clickable text.#marginnote[Blue text, while also a
widely recognizable clickable-text indicator, is crass and distracting.
Luckily, it is also rendered unnecessary by the use of underlining.]

As always, these design choices are merely one approach that Tufte CSS
provides by default. Other approaches can also be made to work. The goal is
to make sentences readable without interference from links, as well as to
make links immediately identifiable even by casual web users.
...
#epigraph(
    ...
    author: [Richard P. Feynman, _"What Do You Care What Other People Think?"_],
)

#epigraph(
    [I do not paint things, I paint only the differences between things.],
    author: [Henri Matisse, _Henri Matisse Dessins: thèmes et variations_ (Paris, 1943), 37],
)
...
(particularly _Beautiful Evidence_), these are `blockquote` elements with
a bit of specialized styling. Quoted text is italicized. The source goes
in a `footer` element inside the `blockquote`. We have provided three
examples in the epigraph of this section, demonstrating shorter and longer
quotes, with and without a paragraph tag, and showing how multiple quotes
within an epigraph fit together with the use of a wrapper class.
...
margin notes, and small figures. On smaller viewports, elements that would
go in the margin are hidden until the user toggles them into view. The goal
is to present related but not necessary information such as asides or
citations _as close as possible_ to the text that references them. At the
same time, this secondary information should stay out of the way of the
eye, not interfering with the progression of ideas in the main text.

Sidenotes consist of two elements: a superscript reference number that goes
inline with the text, and a sidenote with content. To add the former, just
put a label and dummy checkbox into the text where you want the reference
to go, like so:
...
sidenote content itself, in a `span` with class `sidenote`. This tag is
also inserted directly in the middle of the body text, but is either pushed
into the margin or hidden by default. Make sure to position your sidenotes
correctly by keeping the sidenote-number label close to the sidenote itself.
...
margin note.#marginnote[This is a margin note. Notice there isn't a number
preceding the note.] On large screens, a margin note is just a sidenote
that omits the reference number. This lessens the distracting effect taking
away from the flow of the main text, but can increase the cognitive load of
matching a margin note to its referent text. However, on small screens, a
margin note is like a sidenote except its viewability-toggle is a symbol
...
Figures in the margin are created as margin notes, as demonstrated in the
next section.

= Figures

Tufte emphasizes tight integration of graphics with text. Data, graphs, and
figures are kept with the text that discusses them. In print, this means
they are not relegated to a separate page. On the web, that means
readability of graphics and their accompanying text without extra clicks,
tab-switching, or scrolling.

Figures should try to use the `figure` element, which by default are
constrained to the main column. Don't wrap figures in a paragraph tag. Any
label or margin note goes in a regular margin note inside the figure. For
...
those cases, a margin figure may be most appropriate. To place figures in
the margin, just wrap an image (or whatever) in a margin note inside a `p`
tag, as seen to the right of this paragraph.#margin-figure(
...
If you need a full-width figure, give it the `fullwidth` class. Make sure
that's inside an `article`, and it will take up (almost) the full width of
...
Technical jargon, programming language terms, and code samples are denoted
with the `code` class, as I've been using in this document to denote HTML.
Code needs to be monospace for formatting purposes and to aid in code
analysis, but it must maintain its readability. To those ends, Tufte CSS
follows GitHub's font selection, which shifts gracefully along the
monospace spectrum from the elegant but rare Consolas all the way to good
old reliable Courier.
...
#link("http://imagequilts.com/")[ImageQuilts]. See the
#link("http://www.edwardtufte.com/bboard/q-and-a-fetch-msg?msg_id=0003wk")[ET forum announcement thread]
